...
\caption{Mechanism assumptions (Current State $\rightarrow$ Target State).
  $^\dagger$Conservative lower bound: accuracy improves continuously with data; customers cannot distinguish a false-positive forecast from a genuine recommendation and retain full decision authority to accept or decline.}
\label{tab:mech}
\centering
\small
...
A9  & PdM forecast accuracy ($a$)          & 0.90            & 89--92\,\% LSTM-RUL~\cite{abougrad2026xai}$^\dagger$; stated for transparency, not a formula input \\
A10 & Proactive-offer conversion ($k$)     & 0.50            & Conserv.\ estimate; consistent with findings that proactive contact aids uptake~\cite{challagalla_proactive_2009,gonzalez_service_2015} \\
--- & Target capture rate ($\cSoll$, deriv.)& ${\approx}0.70$& $\cIst+(1{-}\cIst)\cdot k$ \\
\bottomrule
\end{tabularx}
\end{table}
 
The target capture rate is derived, not assumed: of the leaked share
$(1-\cIst)$, the fraction that converts on a proactive offer ($k$) is
recaptured. Forecast accuracy (A9) does not enter the formula because customers
cannot distinguish a true-positive forecast from a false-positive one---both
result in a service recommendation they may accept or decline at rate $k$:
\begin{align}
\cSoll &= \cIst + (1-\cIst)\cdot k \notag \\
       &= 0.40 + 0.60 \cdot 0.50 \notag \\
       &= 0.40 + 0.30 = 0.70 .
\end{align}
...
conversion. The forecast accuracy (A9) places no binding constraint on the target
state; accuracy improves with data, and a false-positive forecast imposes no
cost---the customer cannot distinguish it from a genuine recommendation and
simply chooses to accept or decline the offer (see Section~\ref{sec:lim}).
...
&\quad\text{(computed on unrounded rates).}
\end{align}
