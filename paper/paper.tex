...
we (i)~establish a quantitative business case for DT/PdM integration in franchised dealerships using a transparent, assumption-driven model anchored to published German market data, (ii)~compare the predictive target state with the interval-based current state along the full customer journey to identify where reactive servicing causes value leakage, and (iii)~derive actionable recommendations for dealerships and OEMs.
...
representative value within it is used; individual prices may fall toward either end, but the sensitivity analysis brackets this residual uncertainty.
...
yet price competition from independent garages and customer inertia toward switching service providers prevent full conversion, making 0.50 a deliberate floor rather than a central estimate.
...
% --- Absatz 1: Interpretation der Ergebnisse ---
% - €55/Fzg/Jahr klingt gering, skaliert aber: 5.000 Fzg × 8 Jahre (A3) → >2 Mio. € kumuliert
% - Touchpoint-Gewinn (+1,0/Jahr) ist der eigentliche Werthebel: jeder Kontakt = Beratungs- und Upselling-Chance
% - Händler-seitig geht es nicht nur um Teile-Marge, sondern um Kundenbindung:
%   dealer-serviced customers kaufen nächstes Fahrzeug deutlich häufiger beim selben Händler (Cox Automotive 2025)
% - Sensitivitätsanalyse zeigt: selbst bei k=0,50 (konservativ) bleibt der Business Case positiv

% --- Absatz 2: Nicht-technische Hürde als eigentlicher Engpass ---
% - Technologie-Stack (DT, ML, OTA) ist erprobt und verfügbar → nicht die Barriere
% - Datenzugang ist die eigentliche Hürde: EU Data Act (ab Sept. 2025), GDPR, OEM-Kooperation nötig
% - OEM entscheidet, ob und in welchem Format Telemetrie an Händler weitergegeben wird
% - Empfehlung: Händler sollten Consent-Infrastruktur (Kundeneinwilligung, CRM-Integration) aktiv aufbauen
%   statt auf OEM-Standardlösung zu warten
% - Tyres-Leck und BEV-Shift (HV-Batterie statt Zündkerzen) als weitere Scope-Risiken nennen

% --- Absatz 3: Implikationen für Praxis und Forschung ---
% Praxis (Händler):
%   - PdM als Erweiterung des CRM denken, nicht als IT-Projekt
%   - Proaktive Kundenansprache + Terminplanung = Werthebel, nicht die Prognose-Genauigkeit selbst
%   - Pilotstart mit Modellen, die OEM-Telemetrie bereits freigeben (z.B. bestimmte Marken über connected-car APIs)
% Forschung:
%   - Modell ist transparent aber nicht empirisch validiert → Pilotprojekt mit echten In-Service-Daten als nächster Schritt
%   - Conversion Rate k=0,50 ist gesetzt, nicht gemessen → Verhaltensstudie zu PdM-Annahme-Raten wäre wertvoll
%   - Framework auf BEV ausweiten (HV-Batterie SoH als dominanter Prognose-Target)
